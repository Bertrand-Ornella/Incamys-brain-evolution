...
PGLS_OB_Br <-gls(OB ~ Body, correlation=corPagel (1,phy=tree_Incamys), data=Incamys.data)
summary(PGLS_OB_Br)
...
#export dataframe
write.csv(Incamys.data,'OB_BM_Incamys_PGLS.csv')

############### Bloxplot residuals OB vs. Body Incamys ############

#Open data with PEQ & residuals
Incamys.data<-read.csv("OB_BM_Incamys_PGLS.csv", header=T, row.names = 1)
...
ggboxplot(Incamys.data,x="Families", y="residuals", fill="Families", 
  ...
  labs(x='Families', y='Residuals OB vs. BM')
