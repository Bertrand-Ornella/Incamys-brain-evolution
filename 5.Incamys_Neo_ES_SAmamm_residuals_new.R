...
OLS_Neo_Br <-gls(Neo ~ Brain, data=Incamys.data)
summary(OLS_Neo_Br) 

#### Now do the predicted and residuals
Incamys.data$predicted <- predict(OLS_Neo_Br)
Incamys.data$residuals <- residuals(OLS_Neo_Br)

#export dataframe
write.csv(Incamys.data,'Neo_ES_Incamys_SA_mamm_OLS.csv')

############### Bloxplot residuals Brain vs. Body Incamys ############

#Open data with PEQ & residuals
Incamys.data<-read.csv("Neo_ES_Incamys_SA_mamm_OLS.csv", header=T, row.names = 1)

##ggplot - boxplot - residuals
ggboxplot(Incamys.data,x="Families1", y="residuals", fill="Families1",
          palette=c("#3D41B6","#FF8F39"),
  ...
  labs(x='Families1', y='Residuals Neo vs. ES')
...
sink("Neo_ES_ANOVA_Incamys_SAMamm_residuals.txt")
...
#Incamys.data$Families1 = factor(Incamys.data$Families1, 
                               #levels = c("Notoungulata","Caviomorpha")) 
