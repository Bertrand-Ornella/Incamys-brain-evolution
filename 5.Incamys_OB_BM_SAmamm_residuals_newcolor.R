...
OLS_OB_Br <-gls(OB ~ Body, data=Incamys.data)
summary(OLS_OB_Br) 
...
#export dataframe
write.csv(Incamys.data,'OB_BM_Incamys_SA_mamm_OLS.csv')

############### Bloxplot residuals Brain vs. Body Incamys ############

#Open data with PEQ & residuals
Incamys.data<-read.csv("OB_BM_Incamys_SA_mamm_OLS.csv", header=T, row.names = 1)
...
ggboxplot(Incamys.data,x="Families1", y="residuals", fill="Families1",
  ...
  labs(x='Families1', y='Residuals OB vs. BM')
