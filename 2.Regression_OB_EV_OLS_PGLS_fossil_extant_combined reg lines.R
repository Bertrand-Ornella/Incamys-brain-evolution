...
#Save dataset
write.csv(Incamys.data,'Incamys.data_Extant_OB_EV_BM.csv')

#Save tree
write.nexus(tree_Incamys, file = "Incamys_Extant_OB_EV_BM_tree.nex")
...
Incamys.data.extant$Brain_volume_mm3<-log10(Incamys.data.extant$Brain_volume_mm3)
names(Incamys.data.extant)[names(Incamys.data.extant) == "Brain_volume_mm3"] <- "Brain"
...
PGLS_line_Br_B_ex <-gls(OB ~ Brain, correlation=corPagel (1,phy=tree_Incamys.extant), data=Incamys.data.extant)
summary(PGLS_line_Br_B_ex) 

#PGLS regression
pgls.fit.PGLS.ex <- predict(PGLS_line_Br_B_ex) #predict values for OB size
predframe.PGLS.extant <- with(Incamys.data.extant, data.frame(Species_name, Brain, OB = pgls.fit.PGLS.ex))

#OLS regression
OLS_line_Br_B_ex <-gls(OB ~ Brain, data=Incamys.data.extant)
summary(OLS_line_Br_B_ex)

gls.fit.Ext <- predict(OLS_line_Br_B_ex) #predict values for OB size
predframe.OLS.extant <- with(Incamys.data.extant, data.frame(Species_name, Brain, OB = gls.fit.Ext))

############################# Regression lines for ALL data #####################

#Create model PGLS regression line for each Group
PGLS_line_Br_B <-gls(OB ~ Brain, correlation=corPagel (1,phy=tree_Incamys), data=Incamys.data)
summary(PGLS_line_Br_B) 

#PGLS regression
pgls.fit.PGLS <- predict(PGLS_line_Br_B) #predict values for OB size
predframe.PGLS <- with(Incamys.data, data.frame(Species_name, Brain, OB = pgls.fit.PGLS))

#OLS regression
OLS_line_Br_B <-gls(OB ~ Brain, data=Incamys.data)
summary(OLS_line_Br_B)

gls.fit.all <- predict(OLS_line_Br_B) #predict values for OB size
predframe.OLS <- with(Incamys.data, data.frame(Species_name, Brain, OB = gls.fit.all))

############### Make graph with PGLS corrected regressions -- OB/Brain

ggplot(Incamys.data, aes(Brain, OB, color = Regression_point)) +
