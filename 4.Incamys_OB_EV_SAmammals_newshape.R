...
#OLS regression
OLSline_OB_B <-gls(OB ~ Brain, data=Incamys.data)
summary(OLSline_OB_B)

gls.fit.OLS <- predict(OLSline_OB_B) #predict values for OB size
predframe.OLS <- with(Incamys.data, data.frame(Species_name, Brain, OB = gls.fit.OLS))

#Make graph with PGLS corrected regressions -- OB/Brain crown vs. stem
ggplot(Incamys.data, aes(Brain, OB, color = Families1)) +
